    ...
    <section className="rounded-3xl border border-slate-200 bg-white px-6 py-12 shadow-sm sm:px-10">
      <div className="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
        <div className="max-w-xl">
          <h2 className="text-3xl font-semibold text-slate-900">{events.title}</h2>
          <p className="mt-3 text-sm text-slate-600 sm:text-base">{events.description}</p>
        </div>
        <Link
        ...
        {events.items.map((item) => (
          ...
          >
            <div className="flex items-start gap-4">
              <div className="flex h-16 w-16 flex-col items-center justify-center rounded-2xl bg-white text-sm font-semibold text-slate-900 shadow">
            ...
            {item.time ? (
              <div className="text-sm font-semibold text-slate-600">{item.time}</div>
            ) : null}
          </Link>
        ))}
      </div>
