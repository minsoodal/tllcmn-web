  ...
  return (
    <section className="relative isolate overflow-hidden bg-white px-6 pb-20 pt-16 sm:rounded-3xl sm:px-10">
      <div className="mx-auto flex max-w-4xl flex-col items-start gap-10 text-left">
        <div>
          <span className="rounded-full bg-slate-900/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.3em] text-soft">
            {hero.eyebrow}
          </span>
          <h1 className="mt-6 text-4xl font-semibold leading-tight text-slate-900 text-strong sm:text-5xl">
            {hero.title}
          </h1>
          <p className="mt-4 max-w-2xl text-base text-strong sm:text-lg">{hero.subtitle}</p>
        </div>
        <div className="flex flex-col gap-3 sm:flex-row">
          ...
          <Link
            ...
            className="inline-flex items-center justify-center rounded-full border border-slate-300 px-6 py-3 text-sm font-semibold text-slate-900 text-strong transition hover:border-slate-400 hover:bg-slate-100"
          >
